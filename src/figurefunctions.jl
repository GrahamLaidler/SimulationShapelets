...
function Figure10()
    ...
    S1 = plot(Multi_s₁[1][1][:,1], Multi_s₁[1][1][:,2].-minimum(Multi_s₁[1][1][:,2]), linetype=:steppost, lw=1.5, xlabel=L"t", ylim=(-0.3,6.2), xlim=(-0.3, 12), xticks=0:2:12, yticks=0:2:6, ylabel=L"s(t) - \min_u s(u)", color=:sienna1, label=L"s_A", xguidefontsize=10, yguidefontsize=10, tickfontsize=6, legendfontsize=6, legend=:right, dpi=600)
    ...
    S2 = plot(Multi_s₂[1][1][:,1], Multi_s₂[1][1][:,2].-minimum(Multi_s₂[1][1][:,2]), linetype=:steppost, lw=1.5, xlabel=L"t", ylim=(-0.3,6.2), xlim=(-0.3, 12), xticks=0:2:12, yticks=0:2:6, ylabel=L"s(t) - \min_u s(u)", color=:plum4, label=L"s_A", xguidefontsize=10, yguidefontsize=10, tickfontsize=6, legendfontsize=6, legend=:topright, dpi=600)
